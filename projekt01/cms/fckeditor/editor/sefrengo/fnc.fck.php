<?php
function fckConfigStringToArray($string) {
...
function fckBuildMenu($featurestring, $selectablestyles) {
	...
	unset($sub);
	if (in_array('clipboardtools', $features)) $sub[] .= "'Cut','Copy','Paste','PasteText','PasteWord'";
	...
	if (count($sub) > 0) $row1 .= '['.implode (', ', $sub)."],\n";
	unset($sub);
	if (in_array('table', $features)) $sub[] .= "'Table','-','TableInsertRowAfter','TableDeleteRows','TableInsertColumnAfter','TableDeleteColumns','TableInsertCellAfter','TableDeleteCells','TableMergeCells','TableHorizontalSplitCell','TableCellProp'";	
	if (count($sub) > 0) $row1 .= '['.implode (', ', $sub)."],\n";
	unset($sub);
	...
	unset($sub);
	if (in_array('align', $features) ) $sub[] .= "'JustifyLeft','JustifyCenter','JustifyRight','JustifyFull'";
	if (count($sub) > 0) $row2 .= '['.implode (', ', $sub)."],\n";
	unset($sub);
	...
	if (in_array('hr', $features)) $sub[] .= "'Rule'";
