...
if (empty($mvars[11100]))
	$mvars[11100]='LC_ALL,de_DE@euro,de_DE,de,ge';

$_AS['temp']['sl_arr'] = explode(',',$mvars[11100]);
@setlocale ($_AS['temp']['sl_arr'][0],
						$_AS['temp']['sl_arr'][1],
						$_AS['temp']['sl_arr'][2],
						$_AS['temp']['sl_arr'][3],
						$_AS['temp']['sl_arr'][4]); 
...
if (empty($_AS['cal_month']) && empty($_AS['cal_month'])) {
	$_AS['cal_month']=(int) $mvars['74206'];
	$_AS['cal_month_sw']=$_AS['cal_month'];
}
if (empty($_AS['cal_year']) && empty($_AS['cal_year_sw'])) {
	$_AS['cal_year']=(int) $mvars['74207'];
	$_AS['cal_year_sw']=$_AS['sw_cal_year'];
}
...
$_AS['config']['filesize_str_b'] = $mvars['500'];
$_AS['config']['filesize_str_kb'] = $mvars['501'];
$_AS['config']['filesize_str_mb'] = $mvars['502'];
$_AS['config']['filesize_decplaces'] = $mvars['510'];
...
if(!empty($mvars[400])) {
  ...
  foreach ($_AS['sorting']['raw_vals'] AS $v) {
    ...
    if (strpos($_AS['sorting_pieces']['0'],'category')!==false)
		...
		if (!empty($_AS['sorting_pieces']['1'])) 
	    $_AS['sorting']['array'][$_AS['sorting_pieces']['0']]=$_AS['sorting_pieces']['1'];
		else
	    $_AS['sorting']['array'][]=$_AS['sorting_pieces']['0'];
...
for ($i=1;$i<36;$i++)
	if ($i>9)
		$_AS['temp']['sortlinkelements']['CTF'.$i]='custom'.$i;
	else
		$_AS['temp']['sortlinkelements']['CTF0'.$i]='custom'.$i;
...
if (!empty($_AS['temp']['sortlinkvalsoldstring'])) {
	...
	foreach ($_AS['temp']['sortlinkvalsoldtemp'] as $v)
		$_AS['temp']['sortlinkvalsold'][substr($v,0,-1)]=$v;
		if (substr($v,-1)=='A')
			$_AS['sorting']['array'][$_AS['temp']['sortlinkelements'][substr($v,0,-1)]]='ASC';
		else if (substr($v,3,4)=='D')
			$_AS['sorting']['array'][$_AS['temp']['sortlinkelements'][substr($v,0,-1)]]='DESC';
}
...
// get custom filter
//
if (strpos($mvars[5],'{savesettings_form}')!==false) {
	$_AS['temp']['submittedsettings']=$_AS['cms_wr']->getVal($_AS['modkey'].'settings');
	$_AS['temp']['savedsettings'] = as_get_val('as_'.$_AS['modkey'].'settings');


	if ($_AS['cms_wr']->getVal($_AS['modkey'].'settings_submit')==='true') {
		as_set_val('as_'.$_AS['modkey'].'settings',$_AS['temp']['submittedsettings']);
		$_AS['temp']['savedsettings']=$_AS['temp']['submittedsettings'];
	} 
	if (!empty($_AS['temp']['savedsettings']))
		$_AS['temp']['savedsettings_arr']=unserialize(urldecode($_AS['temp']['savedsettings']));
	else
		$_AS['temp']['savedsettings_arr']=array();
	
	if (!empty($_AS['temp']['savedsettings_arr']['searchstring']))
		$_AS['temp']['searchstring']=$_AS['temp']['savedsettings_arr']['searchstring'];
}
...
for ($i=1;$i<36;$i++){
	...
	$_AS['temp']['customfiltersubmitted'.$i]=$_AS['cms_wr']->getVal($_AS['modkey'].'cf'.$i);

	if (is_array($_AS['temp']['customfiltersubmitted'.$i])){
		$_AS['temp']['customfiltersubmitted'.$i]=implode('###',$_AS['temp']['customfiltersubmitted'.$i]);
	}
	
	if (!empty($_AS['temp']['savedsettings_arr']['cf'.$i]) && $_AS['cms_wr']->getVal($_AS['modkey'].'cfsubmited')!='true')
		$_AS['temp']['customfilterselected'.$i]=$_AS['temp']['savedsettings_arr']['cf'.$i];
	else
		$_AS['temp']['customfilterselected'.$i]=$_AS['temp']['customfiltersubmitted'.$i];
	...
	if ( ( $_AS['artsys_obj']->getSetting('article_custom'.$i.'_type')=="select2" ||
			   ...
			   $_AS['artsys_obj']->getSetting('article_custom'.$i.'_multi_select')=="true" &&
		 		 !empty($_AS['temp']['customfilters']['custom'.$i]) )
		$_AS['temp']['customfilters']['custom'.$i]='%'.$_AS['temp']['customfilters']['custom'.$i].'%';
...
#		$_AS['collection']->setHideArchived(false);
  ...
  $_AS['temp']['detail'] = $mvars[7];
	...
	$_AS['config']['day2'] = $mvars[11210];
	$_AS['config']['month2'] = $mvars[11211];
  ...
  //Url
	...
	$_AS['temp']['data']['url_back'] = as_url_creator ( $con_side[(empty($_AS['temp']['idcatsideback'])?$idcatside:$_AS['temp']['idcatsideback'])]['link'],
																							array (	'startmonth' => $_AS['config']['startmonth'],
																											...
																											'cf20' => $_AS['temp']['customfilters2']['custom20'],	
																											...
																											'cf30' => $_AS['temp']['customfilters2']['custom30'],	
																											'cf31' => $_AS['temp']['customfilters2']['custom31'],
  ...
  if(($_AS['item']->available() ||  $mvars['73']=='true') && (int) $_AS['item']->_data['online']>0  ) {
		...
		if ($sf_factory->classExists('LIBS', 'Comments') && strpos($mvars[7],'{comments')!==false)
		{
			...
			$_AS['temp']['comments_config'] = array(
				...
				'baseurl' => $_AS['temp']['sf_comments_baseurl'],//Basislink mit allen benötigten Parametern des Artikelsystems
...
#	$_AS['page_nav_items']=count($_AS['collection']->_data);
	...
	if ($_AS['page_nav_items']>$mvars['48'] && $mvars['48']>0){
		...
		$_AS['page_nav']='';
		$_AS['page_nav_adv']='';
		$_AS['page_nav_adv_array']=array();
		$_AS['page_nav_adv_array']['pages']=array();
...
#			if ($mvars[2]==1)	{		
				...
				if($_AS['pager']->getPrevious()){
					...
					$_AS['page_nav_adv_array']['first'] = str_replace('{url}',$_AS['pager_base_url'].'&amp;'.$_AS['modkey'].'page='.$_AS['pager']->getFirst(),$mvars['500010']);
					$_AS['page_nav_adv_array']['previous'] = str_replace('{url}',$_AS['pager_base_url'].'&amp;'.$_AS['modkey'].'page='.$_AS['pager']->getPrevious(),$mvars['500020']);
				} else {
					$_AS['page_nav_adv_array']['first'] = str_replace('{url}',$_AS['pager_base_url'].'&amp;'.$_AS['modkey'].'page='.$_AS['pager']->getFirst(),$mvars['500015']);
					$_AS['page_nav_adv_array']['previous'] = str_replace('{url}',$_AS['pager_base_url'].'&amp;'.$_AS['modkey'].'page='.$_AS['pager']->getPrevious(),$mvars['500025']);
				}
				...
				for($i=0;$i<count($_AS['pager_links']);$i++) {
					...
					if($_AS['pager_link'] == $_AS['page_nav_current_page']){
						...
						$_AS['page_nav_adv_array']['pages'][] = str_replace(array('{url}',
																																			'{number}'),
																																array($_AS['pager_base_url'].'&amp;'.$_AS['modkey'].'page='.$_AS['pager_link'],
																																			$_AS['pager_link']),
																																$mvars['500035']);
					} else {
				...
				if($_AS['pager']->getNext()){
					...
					if (!empty($mvars['44'])) 
					 ...
					 $_AS['page_nav_adv_array']['last'] = str_replace('{url}',$_AS['pager_base_url'].'&amp;'.$_AS['modkey'].'page='.$_AS['pager']->getLast(),$mvars['500050']);
					 $_AS['page_nav_adv_array']['next'] = str_replace('{url}',$_AS['pager_base_url'].'&amp;'.$_AS['modkey'].'page='.$_AS['pager']->getNext(),$mvars['500040']);
				} else {
					 $_AS['page_nav_adv_array']['last'] = str_replace('{url}',$_AS['pager_base_url'].'&amp;'.$_AS['modkey'].'page='.$_AS['pager']->getLast(),$mvars['500055']);
					 $_AS['page_nav_adv_array']['next'] = str_replace('{url}',$_AS['pager_base_url'].'&amp;'.$_AS['modkey'].'page='.$_AS['pager']->getNext(),$mvars['500045']);
				}
...
#			}			
		...
		$_AS['page_nav_numbers'] = implode($mvars['39'],$_AS['page_nav_array']);
		...
		$_AS['page_nav_adv']=str_replace(array(	'{first}',
																						'{previous}',
																						'{pages}',
																						'{next}',
																						'{last}'),
																		array(	$_AS['page_nav_adv_array']['first'],
																						$_AS['page_nav_adv_array']['previous'],
																						implode($mvars['500300'],$_AS['page_nav_adv_array']['pages']),
																						$_AS['page_nav_adv_array']['next'],
																						$_AS['page_nav_adv_array']['last'],),
																		$mvars['500000']);
  ...
  for($iter = $_AS['collection']->get(); $iter->valid(); $iter->next() ) {
    ...
    if($mvars[70]=='true') {
    	...
    	$_AS['temp']['routed_idcatside']=(int) str_replace('idcatside:','',array_search($_AS['temp']['first_routed_side'][0],$_AS['cat_routing']['routings']));	
			...
			if (!empty($_AS['cat_routing']['routings']['idcatside:'.$_AS['temp']['routed_idcatside']]))
				$_AS['temp']['category']=$_AS['cat_routing']['routings']['idcatside:'.$_AS['temp']['routed_idcatside']];
...
#    if($_AS['item']->available())  {
		...
		if ($sf_factory->classExists('LIBS', 'Comments') && strpos($mvars[7],'{comments')!==false)
		{
			...
			$_AS['temp']['comments_config'] = array(
				...
				'baseurl' => $sf_comments_baseurl,//Basislink mit allen benötigten Parametern des Artikelsystems
				'admin_mail_enabled' => $mvars['400011'],
				'admin_mail_adress' => $mvars['400012'],
				'seconds_to_lock_after_entry' => $mvars['400013'],
				'groups_allowed_to_post' => $mvars['400014'],
...
#				$_AS['temp']['data']['comments'] = $_AS['temp']['sf_comments']->getComments($_AS['temp']['comments_config']);
        ...
        if ($k=='category')
	  	...
	  	// global if-statements
			...
			if ($mvars['2']==1) {
				$_AS['temp']['list_output'] =str_replace('{items_count}','',$_AS['temp']['list_output']);
				$_AS['temp']['list_output'] =str_replace('{items_count:all}','',$_AS['temp']['list_output']);
			}

			$_AS['temp']['list_output'] =str_replace('{items_count}',($_AS['pager_limit1']+$ic+1),$_AS['temp']['list_output']);
			$_AS['temp']['list_output'] =str_replace('{items_count:all}',($_AS['pager_limit1']+$ic+1),$_AS['temp']['list_output']);
			$_AS['temp']['list_output'] =str_replace('{items_count:page}',($ic+1),$_AS['temp']['list_output']);

			$_AS['temp']['list_output'] =str_replace('{items_count:oddeven}',($ic)%2,$_AS['temp']['list_output']);

  		$ic++;
			if ( (!empty($_AS['pager_limit2']) && $ic==$_AS['pager_limit2']) || $ic==$_AS['page_nav_items'])
			  $_AS['temp']['list_output'] = preg_replace('#\{hide_on_last_item\}(.*)\{/hide_on_last_item\}#sU','',$_AS['temp']['list_output']);
			else
			  $_AS['temp']['list_output'] = str_replace(array('{hide_on_last_item}','{/hide_on_last_item}'), array('',''), $_AS['temp']['list_output']);
...
if(strpos($_AS['output']['list_body'],'{if_not_idlang=')!==false) {
	...
	foreach ($_AS['temp']['temp_results'][0] as $ek => $ev) {
		...
		if (strpos($mvars[5],'{savesettings_form}')!==false && 
				$mvars['773']=='true' && 
				count(array_filter($_AS['temp']['savedsettings_arr']))<1 &&
				empty($_AS['temp']['searchstring']) &&
				count(array_filter($_AS['temp']['customfilters']))<1 )
			$_AS['output']['list_body'] = str_replace( '{content}','',$_AS['output']['list_body']);

		if(count($_AS['list']['row']) > 0)
			  $_AS['output']['list_body'] = str_replace(array('{if_content}','{/if_content}'), array('',''), $_AS['output']['list_body']);
			else
				$_AS['output']['list_body'] = preg_replace('#\{if_content\}(.*)\{/if_content\}#sU','',$_AS['output']['list_body']);
		...
		$_AS['output']['list_body'] = str_replace( '{page_nav_adv}',$_AS['page_nav_adv'],$_AS['output']['list_body']);	
		$_AS['output']['list_body'] = str_replace( '{page_nav_numbers}',$_AS['page_nav_numbers'],$_AS['output']['list_body']);	
		...
		if(strpos($_AS['output']['list_body'],'{if_page_numbers}')!==false)
			if (!empty($_AS['page_nav_numbers'])) {
				$_AS['output']['list_body'] = str_replace(array('{if_page_numbers}','{/if_page_numbers}'), array('',''), $_AS['output']['list_body']);
			} else {
				$_AS['output']['list_body'] = preg_replace('#\{if_page_numbers\}(.*)\{/if_page_numbers\}#sU','',$_AS['output']['list_body']);
			}  
	...
	if(strpos($_AS['output']['list_body'],'{if_range_date_to}')!==false && !empty($_AS['timestamp_rangeend'])) {
			$_AS['output']['list_body'] = str_replace(array('{if_range_date_to}','{/if_range_date_to}'), array('',''), $_AS['output']['list_body']);
		} else {
			$_AS['output']['list_body'] = preg_replace('#\{if_range_date_to\}(.*)\{/if_range_date_to\}#sU','',$_AS['output']['list_body']);
		}  

	if(strpos($_AS['output']['list_body'],'{if_not_range_date_to}')!==false && empty($_AS['timestamp_rangeend'])) {
			$_AS['output']['list_body'] = str_replace(array('{if_not_range_date_to}','{/if_not_range_date_to}'), array('',''), $_AS['output']['list_body']);
		} else {
			$_AS['output']['list_body'] = preg_replace('#\{if_not_range_date_to\}(.*)\{/if_not_range_date_to\}#sU','',$_AS['output']['list_body']);
		}  

	if(strpos($_AS['output']['list_body'],'{if_range_date_from}')!==false && !empty($_AS['timestamp_rangestart'])) {
			$_AS['output']['list_body'] = str_replace(array('{if_range_date_from}','{/if_range_date_from}'), array('',''), $_AS['output']['list_body']);
		} else {
			$_AS['output']['list_body'] = preg_replace('#\{if_range_date_from\}(.*)\{/if_range_date_from\}#sU','',$_AS['output']['list_body']);
		}  

	if(strpos($_AS['output']['list_body'],'{if_not_range_date_from}')!==false && empty($_AS['timestamp_rangestart'])) {
			$_AS['output']['list_body'] = str_replace(array('{if_not_range_date_from}','{/if_not_range_date_from}'), array('',''), $_AS['output']['list_body']);
		} else {
			$_AS['output']['list_body'] = preg_replace('#\{if_not_range_date_from\}(.*)\{/if_not_range_date_from\}#sU','',$_AS['output']['list_body']);
		}  


	if(	(!empty($_AS['timestamp_rangestart']) && !empty($_AS['timestamp_rangeend'])) || 
			(!empty($_AS['timestamp_rangestart']) && $mvars[74]=='calendar') ){

		if(empty($_AS['timestamp_rangeend']) && $mvars[74]=='calendar') {
			$_AS['timestamp_rangeend']=$_AS['timestamp_rangestart'];
			$_AS['timestamp_rangeend']=$_AS['timestamp_rangeend']+mktime(23,59,59,0,0,0)-mktime(0,0,0,0,0,0);
		}
	  ...
	  $_AS['output']['list_body'] = str_replace( '{range_date_to:year}',date($_AS['config']['year'],$_AS['timestamp_rangeend']),$_AS['output']['list_body']);	
		...
		$_AS['output']['list_body'] = str_replace( '{range_time_from}','',$_AS['output']['list_body']);
		$_AS['output']['list_body'] = str_replace( '{range_time24_from}','',$_AS['output']['list_body']);
		$_AS['output']['list_body'] = str_replace( '{range_time12_from}','',$_AS['output']['list_body']);
		$_AS['output']['list_body'] = str_replace( '{range_date_from}','',$_AS['output']['list_body']);
		$_AS['output']['list_body'] = str_replace( '{range_date_from:day}','',$_AS['output']['list_body']);
		$_AS['output']['list_body'] = str_replace( '{range_date_from:month}','',$_AS['output']['list_body']);
		$_AS['output']['list_body'] = str_replace( '{range_date_from:year}','',$_AS['output']['list_body']);
		$_AS['output']['list_body'] = str_replace( '{range_time_to}','',$_AS['output']['list_body']);
		$_AS['output']['list_body'] = str_replace( '{range_time24_to}','',$_AS['output']['list_body']);
		$_AS['output']['list_body'] = str_replace( '{range_time12_to}','',$_AS['output']['list_body']);
		$_AS['output']['list_body'] = str_replace( '{range_date_to}','',$_AS['output']['list_body']);
		$_AS['output']['list_body'] = str_replace( '{range_date_to:day}','',$_AS['output']['list_body']);
		$_AS['output']['list_body'] = str_replace( '{range_date_to:month}','',$_AS['output']['list_body']);
		$_AS['output']['list_body'] = str_replace( '{range_date_to:year}','',$_AS['output']['list_body']);
	}
...
//
	...
	for ($i=1;$i<36;$i++) {
		$value=$_AS['temp']['customfilters2']['custom'.$i];
		if(empty($value)) {
		  $_AS['output']['list_body'] = preg_replace('#\{if_custom_selected:'.$i.'\}(.*)\{/if_custom_selected:'.$i.'\}#sU','',$_AS['output']['list_body']);
		  $_AS['output']['list_body'] = str_replace(array('{if_not_custom_selected:'.$i.'}','{/if_not_custom_selected:'.$i.'}'), array('',''), $_AS['output']['list_body']);
		} else {
		  $_AS['output']['list_body'] = preg_replace('#\{if_not_custom_selected:'.$i.'\}(.*)\{/if_not_custom_selected:'.$i.'\}#sU','',$_AS['output']['list_body']);
		  $_AS['output']['list_body'] = str_replace(array('{if_custom_selected:'.$i.'}','{/if_custom_selected:'.$i.'}'), array('',''), $_AS['output']['list_body']);
			$_AS['output']['list_body'] = str_replace('{custom_selected:'.$i.'}',urldecode($value),$_AS['output']['list_body']);
		}
	}
...
//
	...
	foreach($_AS['temp']['sortlinkelements'] as $v)
		$_AS['temp']['sortlinkvalsnew'][ $v]= $v.((substr($_AS['temp']['sortlinkvalsold'][$v],-1)=='A')?'D':'A');
		
	for ($i=1;$i<36;$i++){
			if ($i>9)
				$_AS['temp']['sortlinkvalsnew']['CTF'.$i]='CTF'.$i.((substr($_AS['temp']['sortlinkvalsold']['CTF'.$i],-1)=='A')?'D':'A');
			else
				$_AS['temp']['sortlinkvalsnew']['CTF0'.$i]='CTF0'.$i.((substr($_AS['temp']['sortlinkvalsold']['CTF0'.$i],-1)=='A')?'D':'A');
		}
		...
		$_AS['output']['list_body'] = str_replace('{'.$k.'}',
																							...
																							as_url_creator( $con_side[$idcatside]['link'],
																															array('idarticle' => (($mvars['72']=='list')?$_AS['idarticlemem']:''),
																																		...
																																		'cf34' => $_AS['temp']['customfilters2']['custom34'],
																																		'cf35' => $_AS['temp']['customfilters2']['custom35'],
																																		'sort' => $v.((substr($_AS['temp']['sortlinkvalsold'][$v],-1)=='A')?'D':'A')
																																	)  ).
	...
	// save settings
	//
	if (strpos($_AS['output']['list_body'],'{savesettings_form}')!==false) {

			$_AS['temp']['savesettings']['html']['form_start']	= '<form method="post" name="'.
																															$_AS['modkey'].'savesettingsform" action="'.
																															...
																															'">';
			$_AS['temp']['savesettings']['html']['form_end']		= '</form>';


		$_AS['temp']['savesettings_values']=array();
		for ($i=1;$i<36;$i++) 
			$_AS['temp']['savesettings_values']['cf'.$i]=$_AS['temp']['customfilters']['custom'.$i];
		$_AS['temp']['savesettings_values']['searchstring']=$_AS['temp']['searchstring'];
		$_AS['temp']['savesettings_values']['serializedvalues']=urlencode(serialize($_AS['temp']['savesettings_values']));

		$_AS['temp']['savesettings']['html']['elements']='<input name="'.$_AS['modkey'].'settings" id="'.$_AS['modkey'].'settings" type="hidden" value="'.$_AS['temp']['savesettings_values']['serializedvalues'].'"/>
																											<input name="'.$_AS['modkey'].'settings_submit" id="'.$_AS['modkey'].'settings" type="hidden" value="true"/>';

		$_AS['temp']['savesettings']['html']['tpl'] = $mvars['770'];

																											 
		$_AS['temp']['savesettings']['html']['tpl']=str_replace('{save}',
																														'<input type="submit" id="'.$_AS['modkey'].'settings_submit" value="'.$mvars['771'].'"/>',
																														$_AS['temp']['savesettings']['html']['tpl']);
		$_AS['temp']['savesettings']['html']['tpl']=str_replace('{reset}',
																														'<input type="button" id="'.$_AS['modkey'].'settings_submit" value="'.$mvars['772'].'" 
																														onclick="document.getElementById(\''.$_AS['modkey'].'settings\').value=\'\';document.'.$_AS['modkey'].'savesettingsform.submit();"/>',
																														$_AS['temp']['savesettings']['html']['tpl']);


		$_AS['output']['list_body'] = str_replace('{savesettings_form}',$_AS['temp']['savesettings']['html']['form_start'].
																																		$_AS['temp']['savesettings']['html']['elements'].
																																		$_AS['temp']['savesettings']['html']['tpl'].
																																		$_AS['temp']['savesettings']['html']['form_end'],
																							$_AS['output']['list_body']);
		
	} else {
		$_AS['output']['list_body'] = str_replace('{savesettings_form}','',$_AS['output']['list_body']);
	}

	//
	// custom select
	//
	$_AS['temp']['customselect']['html_comb']['complete']=array();
	preg_match_all('#\{customfilter_combinedform:(.*)\}'.'#isU',$_AS['output']['list_body'],$_AS['temp']['customselect']['pregmatch']);
	$_AS['temp']['customselect']['customfilter_elements_arr']=explode(':',$_AS['temp']['customselect']['pregmatch'][1][0]);

	for ($i=1;$i<36;$i++){
		if ( ( strpos($_AS['output']['list_body'],'{customfilter_form:'.$i.'}')!==false || 
		 			 strpos($_AS['output']['list_body'],'{customfilter_combinedform:')!==false ) &&
			...
			if (strpos($_AS['output']['list_body'],'{customfilter_combinedform:')!==false ) {
				// start & end
				if (empty($mvars[2010]))
					$_AS['temp']['customselect']['html']['form_start_comb']	= '<form method="post" name="'.$_AS['modkey'].'customfilterform"  id="'.$_AS['modkey'].'customfilterform" action="'.
																																	as_url_creator( $con_side[(!empty($mvars[108023])?$mvars[108023]:$idcatside)]['link'],
																																									array('idarticle' => (($mvars['72']=='list')?$_AS['idarticlemem']:''),
																																												'idcatsideback' => (($mvars['72']=='list' && !empty($_AS['temp']['idcatsideback']))?$_AS['temp']['idcatsideback']:''),
																																												'monthback' => $_AS['temp']['monthback'],
																																		...
																																		as_url_creator( $con_side[(!empty($mvars[108023])?$mvars[108023]:$idcatside)]['link'],
																																										array('idarticle' => (($mvars['72']=='list')?$_AS['idarticlemem']:''),
																																													...
																																													'monthback' => $_AS['temp']['monthback'],
																																									 				...
																																									 				'searchstring' => $_AS['temp']['searchstring'],
																																													'category' => $_AS['temp']['category'] ,
																																													'cf'.$i => $_AS['temp']['customfilters2']['custom'.$i]																																										
																																												)  ).																											
																																		'">';
			...
			// start & end
			if (empty($mvars[2010]))
				$_AS['temp']['customselect']['html']['form_start']	= '<form method="post" name="'.$_AS['modkey'].'customfilterform'.$i.'" id="'.$_AS['modkey'].'customfilterform'.$i.'" action="'.
																																as_url_creator( $con_side[(!empty($mvars[108023])?$mvars[108023]:$idcatside)]['link'],
																																								array('idarticle' => (($mvars['72']=='list')?$_AS['idarticlemem']:''),
																																											'idcatsideback' => (($mvars['72']=='list' && !empty($_AS['temp']['idcatsideback']))?$_AS['temp']['idcatsideback']:''),
																																											'monthback' => $_AS['temp']['monthback'],
			...
			if (strpos(strtolower($mvars[103023]),'size')===false)
				$_AS['temp']['attr_selsize']=' size="1"';
			else
				$_AS['temp']['attr_selsize']='';
				
			if (strpos(strtolower($mvars[103023]),'multiple')===false)
				$_AS['temp']['customselect']['html']['form_select'][] =	'<select name="'.$_AS['modkey'].'cf'.$i.'" id="'.$_AS['modkey'].'customfilter'.$i.'"'.$_AS['temp']['attr_selsize'].' '.str_replace('{form_name}',$_AS['modkey'].'customfilterform'.$i,$mvars[103023]).'>';
			else
				$_AS['temp']['customselect']['html']['form_select'][] =	'<select name="'.$_AS['modkey'].'cf'.$i.'[]" id="'.$_AS['modkey'].'customfilter'.$i.'"'.$_AS['temp']['attr_selsize'].' '.str_replace('{form_name}',$_AS['modkey'].'customfilterform'.$i,$mvars[103023]).'>';

			if (strpos(strtolower($mvars[193023]),'size')===false)
				$_AS['temp']['attr_selsize']=' size="1"';
			else
				$_AS['temp']['attr_selsize']='';
				
			if (strpos(strtolower($mvars[193023]),'multiple')===false)
				$_AS['temp']['customselect']['html']['form_select_comb'][] =	'<select name="'.$_AS['modkey'].'cf'.$i.'" id="'.$_AS['modkey'].'customfilter'.$i.'"'.$_AS['temp']['attr_selsize'].' '.str_replace('{form_name}',$_AS['modkey'].'customfilterform'.$i,$mvars[193023]).'>';
			else
				$_AS['temp']['customselect']['html']['form_select_comb'][] =	'<select name="'.$_AS['modkey'].'cf'.$i.'[]" id="'.$_AS['modkey'].'customfilter'.$i.'"'.$_AS['temp']['attr_selsize'].' '.str_replace('{form_name}',$_AS['modkey'].'customfilterform'.$i,$mvars[193023]).'>';
    	...
    	if (!empty($_AS['temp']['customdatastatic'])) {
			...
			if (!empty($mvars[2000])) {
				...
				for ($ii=1;$ii<36;$ii++){
					if (!empty($_AS['temp']['customfilters']['custom'.$ii])) {
						$_AS['temp']['sqlparts_sub']=array();
						foreach(array_filter(explode('###',$_AS['temp']['customfilters']['custom'.$ii])) as $vv) {
	      			if (!empty($vv)){
	      				 $_AS['temp']['sqlparts_sub'][]= ' custom'.$ii.' LIKE \''.$vv.'\' ';
	      			}
	      		}
	      		$sql_cf_addon.=' AND ('.implode('OR',$_AS['temp']['sqlparts_sub']).')';
					}
				}
			}
		    ...
		    $db->query($sql);
			  ...
			  $_AS['temp']['customdataarray2']=array_filter($_AS['temp']['customdataarray2']);
				...
				foreach (as_array_merge($_AS['temp']['customdataarray'],$_AS['temp']['customdataarray2']) as $k1 => $v1){
			...
			$_AS['temp']['customselect']['html']['form_select'][]	=	'<option value="" '.
																																...
																																'>'.$mvars[105019].'</option>'."\n";
			$_AS['temp']['customselect']['html']['form_select_comb'][]	=	'<option value="" '.
																																((htmlentities(stripslashes($_AS['temp']['customfilterselected'.$i]), ENT_COMPAT, 'UTF-8') == $k || stripslashes($_AS['temp']['customfilterselected'.$i]) == $k ) ? 'selected="selected"':'').
																																'>'.$mvars[105019].'</option>'."\n";
												
#    	natcasesort($_AS['temp']['customdataarray']);

			$_AS['temp']['customdataarray']=as_natksort($_AS['temp']['customdataarray']);
			...
			if (is_array($_AS['temp']['customdataarray']))
				foreach ( $_AS['temp']['customdataarray'] as $k => $v){
					...
					$_AS['temp']['customselect']['html']['form_select'][]	=	'<option value="'. $k .'" '.
																																		((htmlentities(stripslashes($_AS['temp']['customfilterselected'.$i]), ENT_COMPAT, 'UTF-8') == $k ||
																																		  stripslashes($_AS['temp']['customfilterselected'.$i]) == $k ||
																																		  in_array(stripslashes($k),explode('###',htmlentities(stripslashes($_AS['temp']['customfilterselected'.$i]), ENT_COMPAT, 'UTF-8'))) ) ? 'selected="selected"':'').
																																		'>'.
																																		htmlentities($v, ENT_COMPAT, 'UTF-8').
																																		'</option>'."\n";
					$_AS['temp']['customselect']['html']['form_select_comb'][]	=	'<option value="'. $k .'" '.
																																		((htmlentities(stripslashes($_AS['temp']['customfilterselected'.$i]), ENT_COMPAT, 'UTF-8') == $k ||
																																		  stripslashes($_AS['temp']['customfilterselected'.$i]) == $k ||
																																		  in_array(stripslashes($k),explode('###',htmlentities(stripslashes($_AS['temp']['customfilterselected'.$i]), ENT_COMPAT, 'UTF-8'))) ) ? 'selected="selected"':'').
																																		'>'.
			...
			$_AS['temp']['customselect']['html']['form_select_comb'][] = '</select>';
		  ...
		  $_AS['temp']['customselect']['html']['form_submit'] = '<input type="submit" value="'.$mvars[105023].'" '.str_replace('{form_name}',$_AS['modkey'].'customfilterform'.$i,$mvars[107023]).' />';
			...
			if (in_array($i,$_AS['temp']['customselect']['customfilter_elements_arr'])) {
				$_AS['temp']['customselect']['html_comb']['complete'][$i] = str_replace(
																											  			array(
																											          '{custom_select}',
																											          '{custom_label}',
																											          '{custom_submit}'
																											        ...
																											        array(
																											          "\n".implode("\n",$_AS['temp']['customselect']['html']['form_select_comb']),
																											          $_AS['temp']['customselect']['html']['form_label'],
																											          $_AS['temp']['customselect']['html']['form_submit']."\n"
																											        ),
																											        $mvars[23]);
			  $_AS['temp']['customselect']['html_comb']['form_submit'] = '<input type="submit" value="'.$mvars[105023].'" '.str_replace('{form_name}',$_AS['modkey'].'customfilterform'.$i,$mvars[107023]).' />';
		    $_AS['temp']['customselect']['html_comb']['form_submited'] = '<input type="hidden" value="true" name="'.$_AS['modkey'].'cfsubmited"/>';
...
if (count($_AS['temp']['customselect']['customfilter_elements_arr'])>0) {
	$_AS['temp']['customselect']['html']['complete'] = $_AS['temp']['customselect']['html']['form_start_comb'].
	                                                   $_AS['temp']['customselect']['html_comb']['form_submited'].
																							  		 str_replace(
																								  			array(
																								          '{custom_selects}',
																								          '{custom_submit}'
																								        ...
																								        array(
																								          "\n".implode("\n",$_AS['temp']['customselect']['html_comb']['complete']),
																								          $_AS['temp']['customselect']['html_comb']['form_submit']."\n"
																								        ),
																								        $mvars[99023]).
																						         $_AS['temp']['customselect']['html']['form_end'];

	$_AS['output']['list_body'] = str_replace('{customfilter_combinedform:'.implode(':',$_AS['temp']['customselect']['customfilter_elements_arr']).'}',$_AS['temp']['customselect']['html']['complete'],$_AS['output']['list_body']);
}
...
/* ]]> */
	...
	//
	// month select 12 month (+/- 6 month)
	//
	if (strpos($_AS['output']['list_body'],'{month2_form}')!==false) {
	
		// start & end
		$_AS['temp']['month2select']['html']['form_start']	= '<form method="post" name="'.
																												...
																												'">';
		$_AS['temp']['month2select']['html']['form_end']		= '</form>';
	
	// select
		$_AS['temp']['month2select']['html']['form_select'][] ='<select name="'.$_AS['modkey'].'startmonth" id="'.$_AS['modkey'].'startmonth" size="1" '.str_replace('{form_name}',$_AS['modkey'].'month',$mvars[103020]).'>';
		...
		for ($i=1;$i<13;$i++) {
			...
			$_AS['temp']['mselect']['avail'][ $current_y+$y][$m]=$m;
			...
			foreach( $_AS['temp']['mselect']['avail'] as $y => $y_arr)
				foreach( $y_arr as $m) {
					...
					$_AS['temp']['month2select']['html']['form_select_from']='';
	
					$_AS['temp']['month2select']['html']['form_select'][] =	'<option value="'.
																																	((-(date('Y')-$y)*12)+$m-1).'" '.
																																	(($_AS['temp']['startmonth'] == ((-(date('Y')-$y)*12)+$m-1) || (empty($_AS['temp']['startmonth']) && $_AS['config']['startmonth']==((-(date('Y')-$y)*12)+$m-1))) ? 'selected="selected"':'').
																																	'>'.
																																	htmlentities(	$y.' '.$_AS['artsys_obj']->lang->get('month_'.($m)).' ', ENT_COMPAT, 'UTF-8').
																																	'</option>';
			}

			$_AS['temp']['month2select']['html']['form_select'][] = '</select>';

			// label
		  $_AS['temp']['month2select']['html']['form_label'] = '<label for="'.$_AS['modkey'].'startmonth" '.str_replace('{form_name}',$_AS['modkey'].'month',$mvars[104020]).'>'.$mvars[101020].'</label>';
		
			// submit
		  $_AS['temp']['month2select']['html']['form_submit'] = '<input type="submit" value="'.$mvars[105020].'" '.str_replace('{form_name}',$_AS['modkey'].'month',$mvars[107020]).' />';
	
			// complete html
			$_AS['temp']['month2select']['html']['complete'] = $_AS['temp']['month2select']['html']['form_start'].
																										  	str_replace(
																										  			array(
																										          '{month_select}',
																										          '{month_label}',
																										        ...
																										        array(
																										          "\n".implode("\n",$_AS['temp']['month2select']['html']['form_select']),
																										          $_AS['temp']['month2select']['html']['form_label'],
																										          $_AS['temp']['month2select']['html']['form_submit']."\n"
																										        ),
																										        $mvars[20]).
																									        $_AS['temp']['month2select']['html']['form_end'];


		 		$_AS['output']['list_body'] = str_replace('{month2_form}',$_AS['temp']['month2select']['html']['complete'],$_AS['output']['list_body']);
...
#print_r(	$_AS['temp']['mselect']['avail']);
...
#		$_AS['collection']->setLegal(mktime(date('H'),date('i'),0,date('m'),date('d'),date('Y')),mktime(date('H'),date('i'),0,date('m'),date('d'),date('Y')));
  ...
  $_AS['collection'] = new ArticleCollection();
		...
		$_AS['tpl']['calendar']['head'] 			= $mvars[27];
		...
		$_AS['tpl']['calendar']['event'] 		= $mvars[29]."\n";

		$_AS['tpl']['calendar']['dayel'] 		= $mvars[26]."\n";
	    ...
	    $_AS['item'] =& $iter->current();
			...
			if ((int) $_AS['item']->getDataByKey($mvars[74201])!=0) {
				...
				$_AS['temp']['cal']['event_start_month']=date('n', strtotime($_AS['item']->getDataByKey($mvars[74201])));
					...
					$_AS['temp']['cal']['event_start_month'] = ($_AS['temp']['cal']['event_start_month']+($_AS['temp']['cal']['event_start_year']-$_AS['cal_year_sw'])*12)-1;
				...
				if ((int) $_AS['item']->getDataByKey($mvars[74202])!=0 && $mvars[74204]=='false') {
					...
					$_AS['temp']['cal']['event_end_month'] = ($_AS['temp']['cal']['event_end_month']+($_AS['temp']['cal']['event_end_year']-$_AS['cal_year_sw'])*12)+1;
					...
					$_AS['temp']['cal'][$_AS['temp']['cal']['event_start_month']+1]['event_end_day']=$_AS['temp']['cal']['event_end_day'];
				}
 				if ($mvars[74204]=='false')
					for($i=$_AS['temp']['cal']['event_start_day'];$i<=$_AS['temp']['cal']['event_end_day'];$i++)
						$_AS['temp']['cal']['eventdates'][$_AS['temp']['cal']['event_start_month']+1][(int) $i]='true';
				else
					$_AS['temp']['cal']['eventdates'][$_AS['temp']['cal']['event_start_month']+1][$_AS['temp']['cal']['event_start_day']]='true';
...
if(!class_exists('AS_Calendar')) {
		...
		class AS_Calendar extends Calendar
		{
		    function getCalendarLink($month, $year)
		    {
		    	global $_AS,$con_side,$mvars,$idcatside;
						...
						$startmonth=($year-date('Y'))*12;
 						...
 						if ($mvars['74205']=='true')
							return as_url_creator( (empty($mvars['74208'])?$con_side[$idcatside]['link']:$con_side[(int) $mvars['74208']]['link']), 
																				array(	'idarticle' => (($mvars['72']=='list')?$_AS['idarticlemem']:''),
																								'idcatsideback' => (($mvars['72']=='list' && !empty($_AS['temp']['idcatsideback']))?$_AS['temp']['idcatsideback']:''),
	        ...
	        if ($_AS['temp']['cal']['eventdates'][$month][$day] == 'true')
	        {
		...
		$_AS['cal']->setMonthLink(as_url_creator( (empty($mvars['74208'])?$con_side[$idcatside]['link']:$con_side[(int) $mvars['74208']]['link']), 
																				array(	'idarticle' => (($mvars['72']=='list')?$_AS['idarticlemem']:''),
																								'idcatsideback' => (($mvars['72']=='list' && !empty($_AS['temp']['idcatsideback']))?$_AS['temp']['idcatsideback']:''),
		...
		if (strpos($_AS['output']['list_body'],'{calendar')!==false) {
			$_AS['output']['list_body'] = str_replace('{calendar}',$_AS['cal']->getMonthView($_AS['cal_month_sw'], $_AS['cal_year_sw'],1,1,$mvars[3000],0),$_AS['output']['list_body']);
			$_AS['output']['list_body'] = str_replace('{calendar_month}',$_AS['cal']->currentMonth,$_AS['output']['list_body']);
			$_AS['output']['list_body'] = str_replace('{calendar_prev_link}',$_AS['cal']->prevMonthLink,$_AS['output']['list_body']);
			$_AS['output']['list_body'] = str_replace('{calendar_next_link}',$_AS['cal']->nextMonthLink,$_AS['output']['list_body']);
		}

		if (strpos($_AS['output']['list_body'],'{calendar:nohead}')!==false)
			$_AS['output']['list_body'] = str_replace('{calendar:nohead}',$_AS['cal']->getMonthView($_AS['cal_month_sw'], $_AS['cal_year_sw'],1,0,$mvars[3000],0),$_AS['output']['list_body']);
		
		if (strpos($_AS['output']['list_body'],'{calendar:prev:')!==false) {
			$_AS['output']['list_body'] = str_replace('{calendar:prev:nohead}',$_AS['cal']->getMonthView($_AS['cal_month_sw'], $_AS['cal_year_sw'],1,0,$mvars[3000],-1),$_AS['output']['list_body']);
			$_AS['output']['list_body'] = str_replace('{calendar:prev}',$_AS['cal']->getMonthView($_AS['cal_month_sw'], $_AS['cal_year_sw'],1,1,$mvars[3000],-1),$_AS['output']['list_body']);
			$_AS['output']['list_body'] = str_replace('{calendar_month:prev}',$_AS['cal']->currentMonth,$_AS['output']['list_body']);
		}
		
		if (strpos($_AS['output']['list_body'],'{calendar:next:')!==false) {
			$_AS['output']['list_body'] = str_replace('{calendar:next:nohead}',$_AS['cal']->getMonthView($_AS['cal_month_sw'], $_AS['cal_year_sw'],1,0,$mvars[3000],1),$_AS['output']['list_body']);
			$_AS['output']['list_body'] = str_replace('{calendar:next}',$_AS['cal']->getMonthView($_AS['cal_month_sw'], $_AS['cal_year_sw'],1,1,$mvars[3000],1),$_AS['output']['list_body']);
			$_AS['output']['list_body'] = str_replace('{calendar_month:next}',$_AS['cal']->currentMonth,$_AS['output']['list_body']);
		}
	...
	$_AS['output']['list_body'] = str_replace('{container_id}',$cms_mod['container']['id'],$_AS['output']['list_body']);
	$_AS['output']['list_body'] = str_replace('{mod_key}',$cms_mod['key'],$_AS['output']['list_body']);
	
	
	//
	//output
	// 
	$_AS['ouput']['final'] = stripslashes($_AS['output']['list_body']);

	echo $_AS['ouput']['final'];
}
