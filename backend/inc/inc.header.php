...
$sql = "SELECT DISTINCT *
        FROM ". $cms_db['clients'] ." A LEFT JOIN ". $cms_db['clients_lang']." B USING(idclient)
        ORDER BY A.name";
$db->query($sql);
...
$sql = "SELECT A.idlang, A.name
        FROM ".$cms_db['lang']." A LEFT JOIN ".$cms_db['clients_lang']." B USING(idlang)
        WHERE B.idclient='$client' ORDER BY idlang";
$db->query($sql);
...
if($con_more_than_one_lang)
{
	...
	$tpl_frm['LANG_FORM'] .= "<input type=\"hidden\" name=\"change_show_tree\" value=\"$show_tree\" />";
	$tpl_frm['LANG_FORM'] .= "\n\n<select name=\"lang\" size=\"1\" onchange=\"languageform.submit()\">";
	$tpl_frm['LANG_FORM'] .= $con_lang_options."</select></form>";
}

$sql = "SELECT idbackendmenu, parent, sortindex, entry_langstring, entry_url, url_target, entry_validate
        FROM ". $cms_db['backendmenu'] ."
        WHERE idclient IN(0, $client) AND entry_langstring NOT IN('empty_dummy') ORDER BY parent, sortindex";
$db->query($sql);
...
// check for active menu layer CHANGE STAM BB
	  if((preg_match('/area='.$layer_cutter.'\b/', $tree[$i]['url']) && empty($cms_plugin) && empty($idplugin) ) 
			|| $idplugin == $tree[$i]['id'] || strstr($tree[$i]['url'], 'cms_plugin='.$cms_plugin) && ! empty($cms_plugin))
	  {
