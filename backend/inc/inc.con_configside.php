...
if($action == 'save')
{
    ...
    if($_REQUEST['rewrite_use_automatic'] != '1')
    {
			if(! rewriteUrlIsAllowed($_REQUEST['rewrite_url'], true))
			{
				...
				$action = 'change';
			}elseif(!rewriteUrlIsUnique('idcatside', $idcatside, $_REQUEST['rewrite_url'])){
				$sf_is_rewrite_error = true;
				$sf_rewrite_error_message = $cms_lang['err_rewrite_Unique'];
				$action = 'change';				
...
echo "x";
*/
$idcatnew[] = $SF_pageinfos->getParent($idcatside);
...
$idsidelang            = $SF_pageinfos->getIdSideLang($idcatside);
$title                 = $SF_pageinfos->getTitle($idcatside);
$author                = $SF_pageinfos->getAuthor($idcatside);
$created               = $SF_pageinfos->getCreatedTimestamp($idcatside);
$summary               = $SF_pageinfos->getSummary($idcatside);
$lastmodified          = $SF_pageinfos->getLastmodifiedTimestamp($idcatside);
$startdate             = date('d.m.Y', $SF_pageinfos->getStart($idcatside));
$starttime             = date('H:i', $SF_pageinfos->getStart($idcatside));
$enddate               = date('d.m.Y', $SF_pageinfos->getEnd($idcatside));
$endtime               = date('H:i', $SF_pageinfos->getEnd($idcatside));
$online                = $SF_pageinfos->getIsOnline($idcatside);
/**
echo 'Debug';
echo $SF_pageinfos->getIsOnline($idcatside);
echo '--';
echo $SF_pageinfos->getIsProtected($idcatside);
*/
$userprotected         = $SF_pageinfos->getIsProtected($idcatside);
$meta_author           = $SF_pageinfos->getMetaAuthor($idcatside);
$meta_description      = $SF_pageinfos->getMetaDescription($idcatside);
$meta_keywords         = $SF_pageinfos->getMetaKeywords($idcatside);
$meta_robots           = $SF_pageinfos->getMetaRobots($idcatside);
$meta_redirect         = $SF_pageinfos->getMetaRedirect($idcatside);
$meta_robots_time      = $SF_pageinfos->getMetaRobotsTime($idcatside);
$meta_redirect_url     = $SF_pageinfos->getMetaRedirectUrl($idcatside);
$rewrite_use_automatic = $SF_pageinfos->getRewriteUseAutomatic($idcatside);
$rewrite_url           = $SF_pageinfos->getRewriteUrlRaw($idcatside);	
...
$select_sidemove = '<select name="idcatnew[]" multiple="multiple" >';
		...
		if($con_tree[$a]['idtplconf'] != '0' && $perm -> have_perm(1, 'cat', $a) || $a == $idcat)
		{
			...
			for($i=0; $i < $catlist_level[$a]; $i++)
			  ...
			  if(!in_array($a,$idcatnew))
			    $select_sidemove .= "<option value=\"$a\">$spaces ".$con_tree[$a]['name']."</option>";
			  else{
...
$radio_visibllity = array(
	                   "0"=> array(
	                         "name"  => $cms_lang['con_side_offline']),
	                   "1"=> array(
	                         "name"  => $cms_lang['con_side_online']),
	                   "2"=> array(
	                         "name"  => $cms_lang['con_side_time'])
                          );
...
$html_startdate = '<input type="date" name="startdate" size="10" maxlength="10" onchange="document.editform.online[2].checked=true" value="'. $startdate .'" />' 
...
$html_enddate = '<input type="date" name="enddate" value="'. $enddate .'" size="10" maxlength="10"  onchange="document.editform.online[2].checked=true;" />
	' . $cms_lang['con_timemanagement_endtime'] . '
	<input type="datetime" name="endtime" value="'. $endtime .'" size="5" maxlength="5"  onchange="document.editform.online[2].checked=true;" />
';	
*/
$html_date =array(
	           "0"=> array(
...
$html_robots=array(
	           "0"=> array(
	                 "value" =>"index, follow",
	                 "name"  => $cms_lang['con_metarobotsif']),
	            "1"=> array(
	                 "value" =>"index, nofollow",
	                 "name"  => $cms_lang['con_metarobotsin']),
	            "2"=> array(
	                  "value" =>"noindex, follow",
	                  "name"  => $cms_lang['con_metarobotsnf']),
	            "3"=> array(
	                  "value" =>"noindex, nofollow",
	                  "name"  => $cms_lang['con_metarobotsnn'])
	                 );
...
$tpl->loadTemplatefile('side_config.tpl',true,true);
...
if($cfg_client['url_rewrite'] == '2' && $have_rewrite_perm)
{
	...
	$tpl_data['SIDE_REWRITE_TITEL'] = $cms_lang['con_rewrite_sidetitel'];
  $tpl_data['SIDE_REWRITE_AUTO'] = $cms_lang['con_rewrite_sideauto'];
	$tpl_data['SIDE_REWRITE_URL'] = $cms_lang['con_rewrite_sideurl'];
	
	if($rewrite_use_automatic == 1)
	{
		$tpl_data['REWRITE_CURRENT_URL'] = ($rewrite_url == '') ? rewriteGetPath($idcat, $lang). '<em>xxx</em>'. $cfg_client['url_rewrite_suffix']: rewriteGetPath($idcat, $lang).$rewrite_url. $cfg_client['url_rewrite_suffix'];
	}else{
		$tpl_data['REWRITE_CURRENT_URL'] = ($rewrite_url == '') ?  '<em>{Diese Seite}</em>':$rewrite_url;
	}
...
if($have_online_offline_perm)
{
	...
	$tpl_data['LANG_ZEIT'] = $cms_lang['con_timemanagement_starttime'];
	$tpl_data['LANG_OFFLINE'] = $cms_lang['con_side_is_offline_at'];
	//$tpl_data['ENDDATE'] = $html_enddate;
	//$tpl->setVariable($tpl_data);
	$tpl_data['STARTDATE_VALUE'] = $html_date[0]['value'];
	$tpl_data['STARTDATE_NAME']  = $html_date[0]['name'];
	$tpl_data['STARTTIME_VALUE'] = $html_date[1]['value'];
	$tpl_data['STARTTIME_NAME']  = $html_date[1]['name'];
	$tpl_data['ENDDATE_VALUE']   = $html_date[2]['value'];
	$tpl_data['ENDDATE_NAME']    = $html_date[2]['name'];
	$tpl_data['ENDTIME_VALUE']   = $html_date[3]['value'];
	$tpl_data['ENDTIME_NAME']    = $html_date[3]['name'];
	$tpl->parseCurrentBlock();
	
	for($j=0; $j<3 ; $j++)
  {
	$tpl->setCurrentBlock('VISIBILITY_BLOCK');
	  if(((int)$online & 0x03)=='0x0'.$j)
    {
 	    $tpl->setVariable(array("VISIBILITY_SELECT"=> 'checked'));
    }else{
      $tpl->setVariable(array("VISIBILITY_SELECT"=> ' '));
    }
	$tpl->setVariable(array("VISIBILITY_ZAHL"=> $j));
	$tpl->setVariable(array("VISIBILITY_NAME"=> $radio_visibllity[$j]['name']));
	$tpl->parseCurrentBlock('VISIBILITY_BLOCK');
	}
	$tpl->setVariable($tpl_data);
	unset($tpl_data);
}else{
...
if($have_meta_perm)
{
  ...
  $tpl->parseCurrentBlock('META');
  for($j=0; $j<4 ; $j++)
  {
    $tpl->setCurrentBlock('META_ROBOTS');
    if($meta_robots==$html_robots[$j]['value'])
    {
 	    $tpl->setVariable(array("META_ROBOTS_SELECT"=> ' selected="selected"'));
    }else{
      $tpl->setVariable(array("META_ROBOTS_SELECT"=> ' '));
    }
    $tpl->setVariable(array("META_ROBOTS_VALUE"=> $html_robots[$j]['value']));
    $tpl->setVariable(array("META_ROBOTS_NAME"=> $html_robots[$j]['name']));
    $tpl->parseCurrentBlock();
  }
  $tpl->setVariable($tpl_data);
  unset($tpl_data,$html_robots);
}else{
	$tpl->setCurrentBlock('HIDDEN_FIELDS');
...
// Rechtemanagement
if(!empty($idcatside) && ( $perm->have_perm(22, 'side', $idcatside, $idcat)  
...
//backendperms
	if($perm->have_perm(22, 'side', $idcatside, $idcat))
	{
		...
		if(!empty($panel1))
		{
$tpl->setVariable('SIDE_RIGHTS',$cms_lang['con_edit']);
			$tpl_data['BACKENDRIGHTS'] = implode("", $panel1);
		}
	}else{
		$tpl->setVariable('SIDE_RIGHTS','');
...
// Einstellungen suchen
	if($configtpl == $idtpl)
	{
		$sql = "SELECT 
		          ...
		          C.idmod, C.version, C.verbose, C.cat, C.source_id,C.idmod
...
// Containername
               ...
               printf ("  <p class=\"moduldesc\">%s\n", (!empty($list[$key]['title'])) ? $list[$key]['title']:"$key. ".$cms_lang['tpl_container']."");
					echo "        <label title=\"". $modtitel . "\"> ".$modname."</label>";
...
$tpl_data['BUTTONS_SEITENANGABEN'] = $tpl_data['BUTTONS_METAANGABEN'] = $tpl_data['BUTTONS_MODULANGABEN'] = $buttons;

//BRAUCH man das dann noch ???
if(!$have_config_tpl_perm && !$have_enter_tpl_perm && ! $have_meta_perm)
{
	$tpl_data['BUTTONS_METAANGABEN'] = '';
}
...
$tpl_data['BACKEND_LANG'] = $cfg_cms['backend_lang'];
$tpl->setVariable($tpl_data);
unset($tpl_data);
